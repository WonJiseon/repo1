/* 주제: 리스터 -  개요 및 ServletContextListener
 ...
 *    - 웹 애플리케이션이 시작되거나 종료될 때 호출될 메서드를 정의한 인터페이스
 *    - 이 인터페이스의 구현체는 웹 애플리케이션이 시작되거나 종료될 때마다 실행된다.
 *    - 모든 서블릿을 위한 공용 도구를 준비할 때 주로 사용한다.
 * 
 * => 리스너도 서블릿과 필터처럼 웹 어플리케이션 DD파일에 등록해야 한다.
 *    또는 애노테이션을 붙여야 한다.
 */
package listener;
